...
void handleNewChar(char c, RobotVelocity &robotVelocity, std::array<uint8_t, MOTOR_COMMAND_SIZE> &motor_command, KickerState &kicker_state, HardwareSerial &robotSerial, bool &stop_dribbler_on_next_command, MotionCommand &current_cmd, float wheel_velocities[4], CommsBuffer &commsBuffer, PacketInfo &packet_info)
{
    ...
    if (c == '\n' || c == '\0')
    {
        ...
        {                     // if buffer not empty
            parseMsg(commsBuffer.buffer, robotVelocity, motor_command, kicker_state, robotSerial, stop_dribbler_on_next_command, current_cmd, wheel_velocities, commsBuffer, packet_info); // process the message
    ...
    else
    {
        ...
        {                        // prevent overflow
            handleNewChar('\0', robotVelocity, motor_command, kicker_state, robotSerial, stop_dribbler_on_next_command, current_cmd, wheel_velocities, commsBuffer, packet_info); // force terminate the string
        }
    }
}
...
void parseMsg(char *msg, RobotVelocity &robotVelocity, std::array<uint8_t, MOTOR_COMMAND_SIZE> &motor_command, KickerState &kicker_state, HardwareSerial &robotSerial, bool &stop_dribbler_on_next_command, MotionCommand &current_cmd, float wheel_velocities[4], CommsBuffer &commsBuffer, PacketInfo &packet_info)
{
    ...
    else if (sscanf(msg, RELEVANT_FORMAT, &commsBuffer.cmd_buffer, &packet_info.bytesParsed) == 1)
    {
        ...
        parseCommand(commsBuffer.cmd_buffer, msg, robotVelocity, motor_command, kicker_state, robotSerial,stop_dribbler_on_next_command, current_cmd, wheel_velocities, commsBuffer, packet_info);
    }
